...
##                        thorpy.makeup.add_basic_help(g_button, race.unit_descr[unit_type], jail=True)
...
##        set_theme("classic")
        ...
        some_race = self.game.players[0].race.name
        choices = self.choices[some_race].values()
        els = [e[-1] for e in choices] + [self.title, self.line]
        h = int(1.1*sum([e.get_fus_size()[1] + 20 for e in els]))
        self.box.set_size((400,h))
        self.box.center()
##        set_theme("human")
...
class BuildingConstructionGUI:
    def __init__(self, game):
        ...
        for what in self.game.construction_price:
            ...
            price = "  " + str(price)+" $"
            time = "  " + str(self.game.construction_time[what])+" turns"
...
##        set_theme("classic")
        ...
        self.line = thorpy.Line(300, "h")
        self.box = thorpy.Element()
        els = [e[0] for e in self.choices.values()] + [self.title, self.line]
        h = int(1.1*sum([e.get_fus_size()[1] for e in els]))
        self.box.set_size((400,h))
        self.box.center()
##        set_theme("human")
...
##                                    self.check_interact_gotocell)]
        ...
        self.game.initialize_last_parameters()
        self.last_move = None
...
##                                                self.life_font_color)
                ...
                if u.is_building:
                    ...
                    text = self.font_life.render(time_left+"t", True, font_color)
##                                                 self.life_font_color)
                    x,y = self.game.me.cam.get_rect_at_coord(u.is_building).center
                    coord = x+4, y+4
                    self.lifes.append((text, (x,y)))
        for coord, n in self.game.burning.items():
            cell = self.game.get_cell_at(coord[0], coord[1])
            font_color = self.font_colors_mat[cell.material.name]
            text = self.font_life.render(str(n), True, font_color)
            x,y = self.game.me.cam.get_rect_at_coord(coord).center
            coord = x+4, y+4
            self.lifes.append((text, coord))
...
##                        self.surface.blit(text.get_image(), rect)
    ...
    def draw_after_objects(self, s):
        ...
        for unit in self.game.units:
            x,y = unit.get_current_cell_rect_center(s)
            if unit.is_building:
