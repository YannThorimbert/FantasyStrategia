...
##                self.refresh_lifes()
##        elif unit in self.blue_highlights: #can help unit
##            pass

    def attack(self):
        defender = self.interaction_objs[0].cell.unit
        print("HEHOO", defender, self.selected_unit)
    ...
    def set_flag(self):
        self.remove_flag()
        cell = self.interaction_objs[0].cell
    ...
    def get_interaction_choices(self, objs):
        ...
        if cell.unit and not(self.selected_unit is cell.unit):
            other = cell.unit
            distance = other.distance_to(self.selected_unit)
            if cell.unit in self.red_highlights: #then self.selected_unit is the agressor
                if distance > 1 and self.selected_unit.attack_range[1] >= distance: #distant attack
                    choices.append(("Distant attack", self.attack))
                elif distance == 1:
                    choices.append(("Attack", self.attack))
            elif cell.unit in self.blue_highlights:
                if distance > 1 and self.selected_unit.help_range[1] >= distance: #distant attack
                    choices.append(("Distant help", self.help))
                elif distance == 1:
                    choices.append(("Help", self.help))
        for o in objs:
            if o is not cell.unit:
                print("self actions", self.actions)
                for name, func in self.actions.get(o.type_name):
                    choices.append((name, func))
    ...
    def user_make_choice(self, choices):
        ...
        class Clicked:
            choice = ""
        for title,func in choices:
            def myfunc():
                Clicked.choice = title
            buttons.append((title,myfunc))
        thorpy.launch_blocking_choices("Choose an action", buttons)
        execute function associated with Clicked.choice
    ...
    def lmb(self, e):
        ...
        if cell:
            if self.destinations_lmb: #then the user may be clicking a destination
                interactibles = self.game.can_interact(cell.coord[0],cell.coord[1])
                if interactibles:
